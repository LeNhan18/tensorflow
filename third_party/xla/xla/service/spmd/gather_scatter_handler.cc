...
PartitionGatherExplicitBatchOrIndexParallelDimensions(
  ...
  } else if (std::optional<hlo_sharding_util::GatherScatterParallelDims>
                 index_parallel_dims =
                     hlo_sharding_util::GetGatherParallelBatchDims(
  ...
  if (!dnums.input_batching_dims().empty()) {
    ...
    parallel_dims.indices_parallel_dims.assign(
        ...
        dnums.scatter_indices_batching_dims().end());
                 ...
                 index_parallel_dims =
                     hlo_sharding_util::GetScatterParallelBatchDims(
