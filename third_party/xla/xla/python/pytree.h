...
class SequenceKey {
 public:
  explicit SequenceKey(int index) : index_(index) {};
  std::string ToReprString() const;
  std::string ToString() const;
  inline int idx() const { return index_; }
  int index_;
  template <typename H>
  inline friend H AbslHashValue(H h, const SequenceKey& n) {
    return H::combine(std::move(h), n.index_);
  };
};
...
class DictKey {
 public:
  ...
  std::string ToReprString() const;
  std::string ToString() const;
  inline nanobind::object key() const { return key_; }
  nanobind::object key_;
  static PyType_Slot slots_[];
  template <typename H>
  inline friend H AbslHashValue(H h, const DictKey& n) {
    return H::combine(std::move(h), nanobind::hash(n.key_));
  };
...
class GetAttrKey {
 public:
  explicit GetAttrKey(std::string_view name) : name_(name) {};
  std::string ToReprString() const;
  std::string ToString() const;
  inline std::string name() const { return name_; }
  const std::string name_;
  template <typename H>
  inline friend H AbslHashValue(H h, const GetAttrKey& n) {
    return H::combine(std::move(h), n.name_);
  };
};
...
class FlattenedIndexKey {
 public:
  ...
  int key_;
  template <typename H>
  inline friend H AbslHashValue(H h, const FlattenedIndexKey& n) {
    return H::combine(std::move(h), n.key_);
  };
};
...
class PyTreeDef {
 ...
 private:
  ...
  // template <typename T>
  // void FlattenImplKP(nanobind::handle handle, T& leaves,
  //                    const std::optional<nanobind::callable>& leaf_predicate,
  //                    std::optional<std::vector<nanobind::object>>& keypath);
