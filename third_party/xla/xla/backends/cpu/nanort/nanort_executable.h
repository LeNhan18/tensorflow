...
class NanoRtExecutable {
 public:
  ...
  // responsibility to make sure that parameters, results and temp buffers are
  // alive during the execution.
  using ExecuteEvent = tsl::Chain;

  // A non-owning read-only view into the XLA executable's parameter buffer.
  class Parameter {
   public:
    template <typename T>
    Parameter(const T* data, int64_t size);

    template <typename T>
    explicit Parameter(absl::Span<const T> data);
  ...
  // A non-owning writable view into the XLA executable's parameter buffer.
  class Result {
   public:
    template <typename T>
  ...
  tsl::AsyncValueRef<ExecuteEvent> Execute(
      absl::Span<const Parameter> parameters, absl::Span<const Result> results,
      const Temp& temp);
...
template <typename T>
NanoRtExecutable::Parameter::Parameter(const T* data, int64_t size)
    : data_(reinterpret_cast<const std::byte*>(data), size * sizeof(T)) {}

template <typename T>
NanoRtExecutable::Parameter::Parameter(absl::Span<const T> data)
    : Parameter(data.data(), data.size()) {}
