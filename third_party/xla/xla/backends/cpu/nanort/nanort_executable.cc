...
static se::DeviceMemoryBase ToDeviceMemory(
    const NanoRtExecutable::Parameter& parameter) {
  return stream_executor::DeviceMemoryBase(
      const_cast<void*>(reinterpret_cast<const void*>(parameter.data().data())),
      parameter.data().size());
}
...
tsl::AsyncValueRef<NanoRtExecutable::ExecuteEvent> NanoRtExecutable::Execute(
  ...
  // Convert parameters, results, and temp to device memory.
  absl::InlinedVector<MaybeOwningDeviceMemory, 8> buffer_device_mem;
  buffer_device_mem.reserve(parameters.size() + results.size() + 1);

  for (const Result& result : results) {
    buffer_device_mem.emplace_back(ToDeviceMemory(result));
  }
  for (const Parameter& parameter : parameters) {
    buffer_device_mem.emplace_back(ToDeviceMemory(parameter));
  }
  buffer_device_mem.emplace_back(ToDeviceMemory(temp));

  // Prepare buffer allocations for parameters, results, and temp.
